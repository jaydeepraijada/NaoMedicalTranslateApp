document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('translation_response', (data) => {
        ...
        // Display translation service information
        const serviceText = data.translation_service === 'openai' ? 
            '<i class="fas fa-robot"></i> Translated by: OpenAI' : 
            '<i class="fas fa-language"></i> Translated by: Google Translate';
        translationService.innerHTML = serviceText;
        translationService.className = `text-muted small mb-2 ${data.translation_service === 'openai' ? 'text-info' : 'text-warning'}`;
        translationService.style.display = 'block';
    ...
    function displayMedicalInfo(data) {
        ...
        if (data.corrections && data.corrections.length > 0) {
            ...
            data.corrections.forEach(correction => {
                content += `<li class="text-info"><i class="fas fa-check-circle"></i> ${correction}</li>`;
            });
            content += '</ul>';
        }
        ...
        if (data.warnings && data.warnings.length > 0) {
            ...
            data.warnings.forEach(warning => {
                content += `<li class="text-warning"><i class="fas fa-exclamation-triangle"></i> ${warning}</li>`;
            });
            content += '</ul>';
        }
        
        // Display found medical terms if any
        if (data.medical_terms_found && data.medical_terms_found.length > 0) {
            content += '<h6 class="mb-2">Detected Medical Terms:</h6><ul class="list-unstyled">';
            data.medical_terms_found.forEach(term => {
                content += `<li><i class="fas fa-heartbeat"></i> ${term.term} (${term.type})</li>`;
            });
            content += '</ul>';
        }
        ...
        // Auto-hide after some time if no warnings
        if (!data.warnings || data.warnings.length === 0) {
            setTimeout(() => {
